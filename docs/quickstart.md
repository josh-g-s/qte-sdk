...
- A session that has closed is not ready straight away. The client waits and asks again as the service says, for up to a minute (`max_wait`), then raises `HistoryPending`. A session still in progress is not served until it closes.
- `HistoryUnavailable` means the data will never exist: a weekend or holiday, a date before the service began, or an instrument or channel it does not know. Other failures raise the other `HistoryError` classes in `qte_sdk.history`, such as `HistoryUnauthenticated` for a bad token.
