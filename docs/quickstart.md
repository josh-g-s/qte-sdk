...
- `HistoryUnavailable` means the data will never exist: a weekend or holiday, a date before the service began, or an instrument or channel it does not know. Other answers from the service raise the other `HistoryError` classes in `qte_sdk.history`, such as `HistoryUnauthenticated` for a bad token. A network failure raises the usual Python error, such as `ConnectionRefusedError`, or `TimeoutError` when the service does not answer within `timeout` seconds (30 by default).
